...
exports.getAvailability = catchAsync(async (req, res, next) => {
  const tour = await Tour.findById(req.params.id).select(
    'name price maxGroupSize startDates duration'
  );
  ...
  // Filter to upcoming, non-sold-out dates
  ...
  for (const sd of rawDates) {
    const locks = await SeatLock.find({ tour: tour._id, startDateId: sd._id.toString() });
